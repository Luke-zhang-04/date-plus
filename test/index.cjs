...
const test = (DatePlus, quiet = false) => {
    ...
    if (dateVals.year !== 2020 || dateVals.month !== 6 || dateVals.day !== 1) {
        throw Error(
            `Word day {year: ${dateVals.year}, month: ${dateVals.month}, day: ${dateVals.day}} from getDateValues does not match {year: 2020, month: 6, day: 1}`,
        )
    }
    ...
    if (date.getElapsedDays(date2) !== 30) {
        throw Error(
            `Elapsed days ${date.getElapsedDays(date2)} from getElapsedDays does not match 30`,
        )
    }
    ...
    if (conv.seconds !== 2 || conv.ms !== 1) {
        throw Error(
            `Converted milliseconds {seconds: ${conv.seconds}, ms: ${conv.ms}} from msToSecs does not match {seconds: 2, ms: 1}`,
        )
    }
    ...
    if (conv.minutes !== 3 || conv.seconds !== 2 || conv.ms !== 1) {
        throw Error(
            `Converted milliseconds {minutes: ${conv.minutes}, seconds: ${conv.seconds}, ms: ${conv.ms}} from msToMins does not match {minutes: 3, seconds: 2, ms: 1}`,
        )
    }
    ...
    if (conv.hours !== 4 || conv.minutes !== 3 || conv.seconds !== 2 || conv.ms !== 1) {
        throw Error(
            `Converted milliseconds {hours: ${conv.hours}, minutes: ${conv.minutes}, seconds: ${conv.seconds}, ms: ${conv.ms}} from msToHrs does not match {hours: 4, minutes: 3, seconds: 2, ms: 1}`,
        )
    }
    ...
    if (
        ...
        throw Error(
            `Converted milliseconds {days: ${conv.days}, hours: ${conv.hours}, minutes: ${conv.minutes}, seconds: ${conv.seconds}, ms: ${conv.ms}} from msToDays does not match {days: 5, hours: 4, minutes: 3, seconds: 2, ms: 1}`,
        )
    }
    ...
    if (DatePlus.secsToMs(12.1) !== 12100) {
        throw Error(
            `Converted seconds ${DatePlus.secsToMs(12.1)} from secsToMs does not match 12100`,
        )
    }
    ...
    if (conv.minutes !== 2 || conv.seconds !== 1) {
        throw Error(
            `Converted seconds {minutes: ${conv.minutes}, seconds: ${conv.seconds}} from secsToMins does not match {minutes: 2 seconds: 1}`,
        )
    }
    ...
    if (conv.hours !== 3 || conv.minutes !== 2 || conv.seconds !== 1) {
        throw Error(
            `Converted seconds {hours: ${conv.hours}, minutes: ${conv.minutes}, seconds: ${conv.seconds}} from secsToHrs does not match {hours: 3, minutes: 2 seconds: 1}`,
        )
    }
    ...
    if (conv.days !== 4 || conv.hours !== 3 || conv.minutes !== 2 || conv.seconds !== 1) {
        throw Error(
            `Converted seconds {days: ${conv.days}, hours: ${conv.hours}, minutes: ${conv.minutes}, seconds: ${conv.seconds}} from secsToDays does not match {days: 4, hours: 3, minutes: 2 seconds: 1}`,
        )
    }
    ...
    if (DatePlus.minsToMs(2.5) !== 150000) {
        throw Error(
            `Converted minutes ${DatePlus.minsToMs(2.5)} from minsToSecs does not match 150000`,
        )
    }
    ...
    if (conv.hours !== 2 || conv.minutes !== 1) {
        throw Error(
            `Converted minutes {hours: ${conv.hours}, minutes: ${conv.minutes}} from minsToHrs does not match {hours: 2, minutes: 1}`,
        )
    }
    ...
    if (conv.days !== 3 || conv.hours !== 2 || conv.minutes !== 1) {
        throw Error(
            `Converted minutes {days: ${conv.days}, hours: ${conv.hours}, minutes: ${conv.minutes}} from minsToDays does not match {days: 3, hours: 2, minutes: 1}`,
        )
    }
    ...
    if (DatePlus.hrsToSecs(2.5) !== 9000) {
        throw Error(
            `Converted hours ${DatePlus.hrsToSecs(2.5)} from hrsToSecs does not match 9000`,
        )
    }
    ...
    if (conv.days !== 2 || conv.hours !== 1) {
        throw Error(
            `Converted hours {days: ${conv.days}, hours: ${conv.hours}} from hrsToDays does not match {days: 2, hours: 1}`,
        )
    }
    ...
    if (DatePlus.daysToMs(2.5) !== 216000000) {
        throw Error(
            `Converted days ${DatePlus.daysToMs(2.5)} from daysToMs does not match 216000000`,
        )
    }
    ...
    if (DatePlus.daysToSecs(2.5) !== 216000) {
        throw Error(
            `Converted days ${DatePlus.daysToSecs(2.5)} from daysToSecs does not match 216000`,
        )
    }
    ...
    if (DatePlus.daysToMins(2.5) !== 3600) {
        throw Error(
            `Converted days ${DatePlus.daysToMins(2.5)} from daysToMins does not match 3600`,
        )
    }
